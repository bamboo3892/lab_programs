...
def model(data, args):
    ...
    for r in pyro.plate("records_rm", Rm):
        sigma = torch.ones([K], device=args.device, dtype=torch.float64) * torch.std(measurements[r]) * 0.1  # [K]
        ...
        # sigma = pyro.param(f"sigma_rm{r}", torch.ones([K], device=args.device, dtype=torch.float64) * torch.std(measurements[r]) * 0.5, constraint=constraints.positive)  # [K]
        ...
        with pyro.plate(f"documents_rm{r}", D) as d:
            ...
            x = pyro.sample(f"x_rm{r}", dist.Normal(mu[z], sigma[z]), obs=measurements[r])  # [D]
...
def guide(data, args):
    ...
    for r in pyro.plate("records_rm", Rm):
        ...
        z_guide = pyro.param(f"z_rm{r}_guide", torch.ones([D, K], device=args.device, dtype=torch.float64) / K, constraint=constraints.simplex)  # [D * K]
        with pyro.plate(f"documents_rm{r}", D) as d:
            z = pyro.sample(f"z_rm{r}", dist.Categorical(z_guide[d]))  # [D]
