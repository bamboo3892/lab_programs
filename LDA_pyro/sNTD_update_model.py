...
def model(data, args):
    ...
    gamma_model = pyro.param("gamma_model", torch.ones([args.K1]), constraint=constraints.positive)
    alpha_model = pyro.param("alpha_model", torch.ones([args.K2]), constraint=constraints.positive)

    with pyro.plate("topics1", args.K1):
        phi = pyro.sample("phi", dist.Dirichlet(beta_model))

    with pyro.plate("season", args.S):
        with pyro.plate("topics2", args.K2):
